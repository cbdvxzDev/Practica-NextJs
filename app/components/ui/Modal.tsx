...
export function Modal({ isOpen, onClose, title, description, children, className }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
      <div className="fixed inset-0 bg-neutral-900/20 backdrop-blur-sm transition-opacity" onClick={onClose} />
      ...
      <div className={cn(
        ...
        <div className="space-y-1.5 pr-6">
          <h2 className="text-base font-medium tracking-tight text-brand-dark">{title}</h2>
          {description && <p className="text-xs text-brand-muted">{description}</p>}
        </div>

        <div className="mt-5 text-sm text-brand-dark">{children}</div>
      </div>
    </div>
  );
}