...
export interface ProductActionsProps {
  productId: string;
  /** Slug del producto, usado para volver a la ficha tras iniciar sesión. */
  slug?: string;
  stock: number;
...
export function ProductActions({
  ...
  const handleWishlistClick = () => {
    if (!isAuthenticated) {
      const back = slug ? `/products/${slug}` : null;
      router.push(back ? `/login?next=${encodeURIComponent(back)}` : "/login");
      return;
    }
