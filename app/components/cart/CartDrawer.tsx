...
/** Mismo contrato que el carrito del store, con imagen y stock opcionales. */
type DrawerCartItem = Omit<CartItemProps, "id" | "image" | "stock"> & {
  lineId: string;
  image?: string;
  stock?: number;
};
        ...
        {items.length > 0 && (
          <div className="border-t border-border/30 p-6 bg-neutral-50/30">
            ...
            <Link
              href="/cart"
              onClick={onClose}
              className="mt-3 block text-center text-[11px] font-medium text-brand-muted underline underline-offset-4 transition-colors hover:text-brand-dark"
            >
              Ver bolsa completa
            </Link>
          </div>
