  ...
  ({ className, type = "text", label, error, id, ...props }, ref) => {
    return (
      <div className="w-full space-y-1.5 text-left">
        {/* Etiqueta flotante o superior sutil */}
        {label && (
          <label 
            htmlFor={id} 
            className="text-xs font-medium text-brand-muted uppercase tracking-wider block"
          >
            {label}
          </label>
        )}
        ...
        {error && (
          <p className="text-xs font-medium text-red-600 animate-fadeIn pl-0.5">
            {error}
          </p>
        )}
      </div>
